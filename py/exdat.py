...
def dist_data(html_code):
    ...
    output_path = os.path.join('output', 'd.json')
    ...
    try:
        ...
        for item in data:
            ...
            if html:
                ...
                print(f"Processing URL: {href_url}")
        ...
        output_path = os.path.join('output', 'p.json')
        with open(output_path, 'w') as json_file:
            json_file.write(json_data)
...
def buyer_data(html_code):
    ...
    json_data = json.dumps(products, indent=2)
    output_path = os.path.join('output', 'b.json')
    with open(output_path, 'w') as json_file:
        json_file.write(json_data)
...
def handle_checkbox_change():
    ...
    html_code = fetch_html_from_file('data.html')  # Assuming fetch_html_from_file is defined in your script
    if html_code:
        if choice == '1':
            dist_data(html_code)
        elif choice == '2':
